...
if (![string]::IsNullOrEmpty($Version)) {
    try {
        $releases = Invoke-WebRequest -UseBasicParsing $releaseUrlBase -Proxy $env:HTTP_PROXY
...
function InstallZip {
    ...
    try {
        ...
        $release = Invoke-WebRequest -UseBasicParsing $releaseUrl -Proxy $env:HTTP_PROXY
        if (!$release) {
