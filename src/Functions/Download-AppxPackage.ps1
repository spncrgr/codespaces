...
# Usage:
# The following command will download the Windows Terminal Preview app with dependencies 
# Download-AppxPackage "https://apps.microsoft.com/detail/9N8G5RFZ9XK3" "$ENV:USERPROFILE\Desktop"
...
param (
  ...
  process {
   # If ProductId was provided, insert that into the standard store URL
   if($ProductId -ne $null) {
    $Uri = "https://apps.microsoft.com/detail/$ProductId"
   }

   # TODO: Better Proxy logic


    $Path = (Resolve-Path $Path).Path
    #Get Urls to download
    $WebResponse = Invoke-WebRequest -UseBasicParsing -Method 'POST' -Uri 'https://store.rg-adguard.net/api/GetFiles' -Body "type=url&url=$Uri&ring=Retail" -ContentType 'application/x-www-form-urlencoded' -Proxy "$env:HTTP_PROXY"
